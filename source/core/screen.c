...
static void screen_set_blend(u32 color, bool rgb, bool alpha) {
    ...
    if(env == NULL) {
        error_panic("Fallo al recuperar la configuracion del combinador.");
        return;
    }
...
void screen_init() {
    if(!C3D_Init(C3D_DEFAULT_CMDBUF_SIZE * 4)) {
        error_panic("Fallo al inicializar la GPU.");
        return;
    }
    ...
    if(target_top == NULL) {
        error_panic("Fallo al inicializar la pantalla superior.");
        return;
    }
    ...
    if(target_bottom == NULL) {
        error_panic("Fallo al inicializar la pantalla inferior.");
        return;
    }
    ...
    if(dvlb == NULL) {
        error_panic("Fallo al parsear el shader.");
        return;
    }

    Result progInitRes = shaderProgramInit(&program);
    if(R_FAILED(progInitRes)) {
        error_panic("Fallo al inicializar el \nprograma de shader: 0x%08lX", progInitRes);
        return;
    }
    ...
    if(R_FAILED(progSetVshRes)) {
        error_panic("Fallo al configurar el \nshader de vertices: 0x%08lX", progInitRes);
        return;
    }
    ...
    if(attrInfo == NULL) {
        error_panic("Fallo al recuperar la informacion \ndel atributo.");
        return;
    }
    ...
    if(R_FAILED(fontMapRes)) {
        error_panic("Fallo al asignar la fuente \ndel sistema: 0x%08lX", fontMapRes);
        return;
    }
    ...
    if(glyph_sheets == NULL) {
        error_panic("Fallo al asignar la textura \nde la fuente.");
        return;
    }
...
void screen_set_color(u32 id, u32 color) {
    if(id >= MAX_COLORS) {
        error_panic("Intento dibujar una cadena con \nun ID de color invalido \"%lu\".", id);
        return;
    }
...
u32 screen_allocate_free_texture() {
    ...
    if(id == 0) {
        error_panic("Sin texturas libres.");
        return 0;
    }
...
static void screen_prepare_texture(u32* pow2WidthOut, u32* pow2HeightOut, u32 id, u32 width, u32 height, GPU_TEXCOLOR format, bool linearFilter) {
    if(id >= MAX_TEXTURES) {
        error_panic("Intento de preparar un ID de \ntextura no valido \"%lu\".", id);
        return;
    }
    ...
    if(textures[id].tex.data == NULL && !C3D_TexInit(&textures[id].tex, (u16) pow2Width, (u16) pow2Height, format)) {
        error_panic("Fallo al inicializar textura \ncon el ID \"%lu\".", id);
        return;
    }
...
void screen_load_texture_path(u32 id, const char* path, bool linearFilter) {
    if(id >= MAX_TEXTURES) {
        error_panic("Intento de cargar ruta \"%s\" a \ntextura invalida con ID \"%lu\".", path, id);
        return;
    }

    FILE* fd = fopen(path, "rb");
    if(fd == NULL) {
        error_panic("Fallo al cargar PNG \"%s\": %s", path, strerror(errno));
        return;
    }
...
void screen_load_texture_file(u32 id, FILE* fd, bool linearFilter) {
    if(id >= MAX_TEXTURES) {
        error_panic("Intento de carga una textura \ninvalida con ID \"%lu\".", id);
        return;
    }
    ...
    if(image == NULL) {
        error_panic("Fallo al carga PNG a textura \ninvalida con ID \"%lu\".", id);
        return;
    }
...
void screen_unload_texture(u32 id) {
    if(id >= MAX_TEXTURES) {
        error_panic("Intento de descargar textura \ninvalida con ID \"%lu\".", id);
        return;
    }
...
void screen_get_texture_size(u32* width, u32* height, u32 id) {
    if(id >= MAX_TEXTURES) {
        error_panic("Intento de obtener el tamaño de \ntextura invalido con ID \"%lu\".", id);
        return;
    }
...
void screen_begin_frame() {
    if(!C3D_FrameBegin(C3D_FRAME_SYNCDRAW)) {
        error_panic("Fallo al crear el frame.");
        return;
    }
}
...
void screen_select(gfxScreen_t screen) {
    ...
    if(!C3D_FrameDrawOn(target)) {
        error_panic("No se pudo seleccionar el destino \nde procesamiento.");
        return;
    }
...
void screen_draw_texture(u32 id, float x, float y, float width, float height) {
    if(id >= MAX_TEXTURES) {
        error_panic("Intento de dibujar una textura \ninvalida con ID \"%lu\".", id);
        return;
    }
...
static void screen_draw_string_internal(const char* text, float x, float y, float scaleX, float scaleY, u32 colorId, bool centerLines, bool wrap, float wrapX) {
    ...
    if(colorId >= MAX_COLORS) {
        error_panic("Intento de dibujar una cadena de color invalido con ID \"%lu\".", colorId);
        return;
    }
