...
static list_item rename_opt = {"Renombrar", COLOR_TEXT, action_rename};
static list_item copy = {"Copiar", COLOR_TEXT, NULL};
static list_item paste = {"Pegar", COLOR_TEXT, action_paste_contents};

static list_item delete_file = {"Borrar", COLOR_TEXT, action_delete_file};

static list_item install_cia = {"Instalar CIA", COLOR_TEXT, action_install_cia};
static list_item install_and_delete_cia = {"Instalar y borrar CIA", COLOR_TEXT, action_install_cia_delete};

static list_item install_ticket = {"Instalar ticket", COLOR_TEXT, action_install_ticket};
static list_item install_and_delete_ticket = {"Instalar y borrar ticket", COLOR_TEXT, action_install_ticket_delete};

static list_item delete_dir = {"Borrar", COLOR_TEXT, action_delete_dir};
static list_item copy_all_contents = {"Copiar todo el contenido", COLOR_TEXT, NULL};
static list_item delete_all_contents = {"Borrar todo el contenido", COLOR_TEXT, action_delete_dir_contents};
static list_item new_folder = {"Nueva carpeta", COLOR_TEXT, action_new_folder};

static list_item install_all_cias = {"Instalar todos los CIAs", COLOR_TEXT, action_install_cias};
static list_item install_and_delete_all_cias = {"Instalar y borrar todos los CIAs", COLOR_TEXT, action_install_cias_delete};
static list_item delete_all_cias = {"Borrar todos los CIAs", COLOR_TEXT, action_delete_dir_cias};

static list_item install_all_tickets = {"Instalar todos los tickets", COLOR_TEXT, action_install_tickets};
static list_item install_and_delete_all_tickets = {"Instalar y borrar todos los tickets", COLOR_TEXT, action_install_tickets_delete};
static list_item delete_all_tickets = {"Borrar todos los tickets", COLOR_TEXT, action_delete_dir_tickets};
    ...
    if(selected != NULL && (selected->data != NULL || selected == &copy || selected == &copy_all_contents) && (selectedTouched || (hidKeysDown() & KEY_A))) {
        ...
        if(selected == &copy || selected == &copy_all_contents) {
            ...
            if(R_SUCCEEDED(res = clipboard_set_contents(actionData->parent->archive, info->path, selected == &copy_all_contents))) {
                prompt_display_notify("Exito", selected == &copy_all_contents ? "Contenido del directorio actual \ncopiado al portapapeles." : (info->attributes & FS_ATTRIBUTE_DIRECTORY) ? "Directorio actual copiado al portapapeles." : "Archivo copiado al portapapeles.", COLOR_TEXT, info, task_draw_file_info, NULL);
            } else {
                error_display_res(info, task_draw_file_info, res, "Fallo al copiar al portapapeles.");
            }
...
static void files_action_open(linked_list* items, list_item* selected, files_data* parent) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Fallo al asignar los datos de \naccion de los archivos.");
    ...
    list_display((((file_info*) selected->data)->attributes & FS_ATTRIBUTE_DIRECTORY) ? "Accion de directorio" : "Accion de archivo", "A: Seleccionar, B: Volver", data, files_action_update, files_action_draw_top);
}
...
static void files_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        files_options_add_entry(items, "Mostrar oculto", &listData->showHidden);
        files_options_add_entry(items, "Mostrar directorios", &listData->showDirectories);
        files_options_add_entry(items, "Mostrar archivos", &listData->showFiles);
        files_options_add_entry(items, "Mostrar CIAs", &listData->showCias);
        files_options_add_entry(items, "Mostrar tickets", &listData->showTickets);
    }
}

static void files_options_open(files_data* data) {
    list_display("Opciones", "A: Cambiar, B: Volver", data, files_options_update, NULL);
}
...
static void files_repopulate(files_data* listData, linked_list* items) {
    ...
    if(R_FAILED(res)) {
        error_display_res(NULL, NULL, res, "Fallo al iniciar el llenano \nde la lista de archivos.");
    }
...
static void files_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(selected != NULL && selected->data != NULL && (selectedTouched || (hidKeysDown() & KEY_A))) {
        ...
        if((fileInfo->attributes & FS_ATTRIBUTE_DIRECTORY) && strncmp(selected->name, "<directorio actual>", LIST_ITEM_NAME_MAX) != 0) {
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "Fallo al completar la lista \nde archivos.");
...
void files_open(FS_ArchiveID archiveId, FS_Path archivePath) {
    ...
    if(R_FAILED(res = fs_open_archive(&data->archive, archiveId, archivePath))) {
        error_display_res(NULL, NULL, res, "Fallo al abrir el archivo de \nla lista de archivos.");

        files_free_data(data);
        return;
    }

    list_display("Archivos", "A: Seleccionar, B: Volver, X: Refrescar, Select: Opciones", data, files_update, files_draw_top);
}
...
void files_open_nand_warning(FS_ArchiveID archive) {
    prompt_display_yes_no("Confirmacion", "Modificar la NAND puede hacer que el\n sistema no funcione.\nAsegurese de lo que esta haciendo.\n\n¿Proceder?", COLOR_TEXT, (void*) archive, NULL, files_open_nand_warning_onresponse);
}
