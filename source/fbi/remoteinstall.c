...
static void remoteinstall_network_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(sock >= 0) {
        ...
        if(remoteinstall_network_recvwait(networkData->clientSocket, &size, sizeof(size), 0) != sizeof(size)) {
            error_display_errno(NULL, NULL, errno, "Fallo al leer la longitud \ndel payload.");
        ...
        if(size >= DOWNLOAD_URL_MAX * INSTALL_URLS_MAX) {
            error_display(NULL, NULL, "Payload demasiado grande.");
        ...
        if(urls == NULL) {
            error_display(NULL, NULL, "Fallo al asignar el buffer de la URL.");
        ...
        if(remoteinstall_network_recvwait(networkData->clientSocket, urls, size, 0) != size) {
            error_display_errno(NULL, NULL, errno, "Fallo al leer la(s) URL(s).");
        ...
        action_install_url("Instalar desde la(s) URL(s) recibida(s)?", urls, NULL, data, NULL, remoteinstall_network_close_client, NULL);
        ...
        error_display_errno(NULL, NULL, errno, "Fallo al abrir el socket.");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Esperando la conexion...\nIP: %s\nPuerto: 5000", inet_ntoa(addr));
}
...
static void remoteinstall_receive_urls_network() {
    ...
    if(bind(data->serverSocket, (struct sockaddr*) &server, sizeof(server)) < 0) {
        error_display_errno(NULL, NULL, errno, "Fallo al vincular el socket \ndel servidor.");
    ...
    if(listen(data->serverSocket, 5) < 0) {
        error_display_errno(NULL, NULL, errno, "Fallo al escuchar en el socket \ndel servidor.");

        remoteinstall_network_free_data(data);
        return;
    }

    info_display("Recibir URL(s)", "B: Volver", false, data, remoteinstall_network_update, NULL);
}
...
static void remoteinstall_qr_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(!installData->capturing) {
        ...
        if(R_FAILED(capRes)) {
            ...
            error_display_res(NULL, NULL, capRes, "Fallo al iniciar la captura \nde la camara.");
    ...
    if(installData->captureInfo.finished) {
        ...
        if(R_FAILED(installData->captureInfo.result)) {
            error_display_res(NULL, NULL, installData->captureInfo.result, "Error al capturar los \nfotogramas de la camara.");
        }
    ...
    for(int i = 0; i < qrCount; i++) {
        ...
        if(err == 0) {
            ...
            action_install_url("¿Instalar desde el codigo QR escaneado?", (const char*) qrData.payload, NULL, NULL, NULL, NULL, NULL);
            return;
        }
    }

    snprintf(text, PROGRESS_TEXT_MAX, "Esperando el codigo QR...");
}
...
static void remoteinstall_scan_qr_code() {
    ...
    if(data->qrContext == NULL) {
        error_display(NULL, NULL, "Fallo al crear el contexto \ndel QR.");
    ...
    if(quirc_resize(data->qrContext, QR_IMAGE_WIDTH, QR_IMAGE_HEIGHT) != 0) {
        error_display(NULL, NULL, "Fallo al redimensionar el \ncontexto del QR.");
    ...
    if(data->captureInfo.buffer == NULL) {
        error_display(NULL, NULL, "Fallo al crear el buffer de imagen.");
    ...
    info_display("Instalacion de codigo QR", "B: Volver, X: Cambiar camara", false, data, remoteinstall_qr_update, remoteinstall_qr_draw_top);
}
...
static void remoteinstall_manually_enter_urls_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        action_install_url("¿Instalar desde la(s) URL(s) ingresada(s)?", response, NULL, NULL, NULL, NULL, NULL);
    }
}

static void remoteinstall_manually_enter_urls() {
    kbd_display("Introduce la(s) URL(s)", "", SWKBD_TYPE_NORMAL, SWKBD_MULTILINE, SWKBD_NOTEMPTY_NOTBLANK, DOWNLOAD_URL_MAX * INSTALL_URLS_MAX, NULL, remoteinstall_manually_enter_urls_onresponse);
}
...
static void remoteinstall_repeat_last_request() {
    ...
    if(textBuf != NULL) {
        if(remoteinstall_get_last_urls(textBuf, DOWNLOAD_URL_MAX * INSTALL_URLS_MAX)) {
            action_install_url("¿Instalar desde la(s) ultima(s) URL (s) ingresada?", textBuf, NULL, NULL, NULL, NULL, NULL);
        } else {
            prompt_display_notify("Error", "No se encontro ninguna URL solicitada anteriormente.", COLOR_TEXT, NULL, NULL, NULL);
        }

        free(textBuf);
    } else {
        error_display_res(NULL, NULL, R_APP_OUT_OF_MEMORY, "Fallo al asignar el buffer \nde texto de la URL.");
    }
}
...
static void remoteinstall_forget_last_request() {
    ...
    if(R_SUCCEEDED(forgetRes)) {
        prompt_display_notify("Exito", "Ultima(s) URL(s) solicitada (s) olvidada(s).", COLOR_TEXT, NULL, NULL, NULL);
    } else {
        error_display_res(NULL, NULL, forgetRes, "Fallo al olvidar la(s) ultima(s) URL(s) solicitada(s).");
    }
}

static list_item receive_urls_network = {"Recibir URL(s) a traves de la red", COLOR_TEXT, remoteinstall_receive_urls_network};
static list_item scan_qr_code = {"Escanear codigo QR", COLOR_TEXT, remoteinstall_scan_qr_code};
static list_item manually_enter_urls = {"Escribir URL manualmente", COLOR_TEXT, remoteinstall_manually_enter_urls};
static list_item repeat_last_request = {"Repetir la ultima solicitud", COLOR_TEXT, remoteinstall_repeat_last_request};
static list_item forget_last_request = {"Olvidar la ultima solicitud", COLOR_TEXT, remoteinstall_forget_last_request};
...
void remoteinstall_open() {
    list_display("Instalacion remota", "A: Seleccionar, B: Volver", NULL, remoteinstall_update, NULL);
}
