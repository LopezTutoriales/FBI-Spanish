...
static list_item delete_pending_title = {"Borrar titulo pendiente", COLOR_TEXT, action_delete_pending_title};
static list_item delete_all_pending_titles = {"Borrar todos los titulos pendientes", COLOR_TEXT, action_delete_all_pending_titles};
...
static void pendingtitles_action_open(linked_list* items, list_item* selected) {
    ...
    list_display("Accion de titulos pendientes", "A: Seleccionar, B: Volver", data, pendingtitles_action_update, pendingtitles_action_draw_top);
}
...
static void pendingtitles_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Fallo al llenar la lista \nde titulos pendientes.");
        }
...
void pendingtitles_open() {
    ...
    list_display("Titulos pendientes", "A: Seleccionar, B: Volver, X: Refrescar", data, pendingtitles_update, pendingtitles_draw_top);
}