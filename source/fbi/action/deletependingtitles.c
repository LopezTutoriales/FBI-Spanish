...
static bool action_delete_pending_titles_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_pending_titles_draw_top, res, "Fallo al borrar el \ntitulo pendiente.");
    return true;
}
...
static void action_delete_pending_titles_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("Exito", "Titulo(s) pendiente(s) borrado(s).", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_pending_titles_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Borrando titulo(s) pendiente(s)", "Pulsa B para cancelar.", true, data, action_delete_pending_titles_update, action_delete_pending_titles_draw_top);
        } else {
...
static void action_delete_pending_titles_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Obteniendo lista de titulos pendientes...");
}
...
void action_delete_pending_titles(linked_list* items, list_item* selected, const char* message, bool all) {
    ...
    if(all) {
        ...
        info_display("Cargando", "Pulsa B para cancelar.", false, loadingData, action_delete_pending_titles_loading_update, action_delete_pending_titles_loading_draw_top);
    } else {
        ...
        prompt_display_yes_no("Confirmacion", message, COLOR_TEXT, data, action_delete_pending_titles_draw_top, action_delete_pending_titles_onresponse);
    }
}

void action_delete_pending_title(linked_list* items, list_item* selected) {
    action_delete_pending_titles(items, selected, "¿Borrar el titulo pendiente seleccionado?", false);
}

void action_delete_all_pending_titles(linked_list* items, list_item* selected) {
    action_delete_pending_titles(items, selected, "¿Borrar todos los titulos pendientes?", true);
}