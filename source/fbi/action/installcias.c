...
static Result action_install_cias_open_dst(void* data, u32 index, void* initialReadBlock, u64 size, u32* handle) {
    ...
    if(R_SUCCEEDED(APT_CheckNew3DS(&n3ds)) && !n3ds && ((info->ciaInfo.titleId >> 28) & 0xF) == 2) {
        ui_view* view = prompt_display_yes_no("Confirmacion", "Titulo creado para consolas NEW 3DS.\n¿Continuar?", COLOR_TEXT, data, action_install_cias_draw_top, action_install_cias_n3ds_onresponse);
        if(view != NULL) {
...
bool action_install_cias_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_install_cias_draw_top, res, "Fallo al instalar archivo CIA.");
    return true;
}
...
static void action_install_cias_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Instalando CIA(s)", "Pulsa B para cancelar.", true, data, action_install_cias_update, action_install_cias_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Fallo al iniciar la \ninstalacion de CIA(s).");
...
static void action_install_cias_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            prompt_display_yes_no("Confirmacion", loadingData->message, COLOR_TEXT, loadingData->installData, action_install_cias_draw_top, action_install_cias_onresponse);
        } else {
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Obteniendo lista de CIA(s)...");
}
...
static void action_install_cias_internal(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData, const char* message, bool delete) {
    ...
    if(R_FAILED(targetCreateRes)) {
        error_display_res(NULL, NULL, targetCreateRes, "Fallo al crear el archivo \nde destino.");
    ...
    info_display("Cargando", "Pulsa B para cancelar.", false, loadingData, action_install_cias_loading_update, action_install_cias_loading_draw_top);
}

void action_install_cia(linked_list* items, list_item* selected) {
    action_install_cias_internal(items, selected, NULL, NULL, "¿Instalar el CIA seleccionado?", false);
}

void action_install_cia_delete(linked_list* items, list_item* selected) {
    action_install_cias_internal(items, selected, NULL, NULL, "¿Instalar y borrar el \nCIA seleccionado?", true);
}
...
void action_install_cias(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData) {
    action_install_cias_internal(items, selected, filter, filterData, "¿Instalar todos los CIAs en \nel directorio actual?", false);
}
...
void action_install_cias_delete(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData) {
    action_install_cias_internal(items, selected, filter, filterData, "¿Instalar y borrar TODOS los CIAs \nen el directorio actual?", true);
}