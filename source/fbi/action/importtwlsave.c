...
static bool action_import_twl_save_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(((import_twl_save_data*) data)->title, task_draw_title_info, res, "Fallo al importar partida.");
    return true;
}
...
static void action_import_twl_save_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(importData->importInfo.finished) {
        ...
        if(R_SUCCEEDED(importData->importInfo.result)) {
            prompt_display_notify("Exito", "Partida importada.", COLOR_TEXT, importData->title, task_draw_title_info, NULL);
        }
...
static void action_import_twl_save_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Importando partida", "Pulsa B para cancelar.", true, data, action_import_twl_save_update, action_import_twl_save_draw_top);
        } else {
            error_display_res(importData->title, task_draw_title_info, res, "Fallo al inicializar el \nimportado de partida.");
            free(data);
        }
    } else {
        free(data);
...
void action_import_twl_save(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Fallo al asignar la importacion \nde partidas de DS/DSi.");
    ...
    prompt_display_yes_no("Confirmacion", "¿Importar partida guardada del titulo seleccionado?", COLOR_TEXT, data, action_import_twl_save_draw_top, action_import_twl_save_onresponse);
}