...
static bool action_delete_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_draw_top, res, "Fallo al borrar contenido.");
    return true;
}
...
static void action_delete_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("Exito", "Borrado.", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Borrando", "Presiona B para cancelar.", true, data, action_delete_update, action_delete_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Fallo al iniciarlizar la \naccion de borrar.");
...
static void action_delete_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            prompt_display_yes_no("Confirmacion", loadingData->message, COLOR_TEXT, loadingData->deleteData, action_delete_draw_top, action_delete_onresponse);
        } else {
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Obteniendo lista de contenido...");
}
...
static void action_delete_internal(linked_list* items, list_item* selected, const char* message, bool recursive, bool includeBase, bool ciasOnly, bool ticketsOnly) {
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "Fallo al llenar la lista \nde contenido.");
    ...
    info_display("Cargando", "Presiona B para cancelar.", false, loadingData, action_delete_loading_update, action_delete_loading_draw_top);
}

void action_delete_file(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "¿Borrar archivo seleccionado?", false, true, false, false);
}

void action_delete_dir(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "¿Borrar directorio actual?", true, true, false, false);
}

void action_delete_dir_contents(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "¿Borrar todo el contenido del directorio actual?", true, false, false, false);
}

void action_delete_dir_cias(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "¿Borrar todos los CIAS del directorio actual?", false, false, true, false);
}

void action_delete_dir_tickets(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "¿Borrar todos los tickets del directorio actual?", false, false, false, true);
}