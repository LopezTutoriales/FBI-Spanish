...
static list_item delete_ticket = {"Borrar Ticket", COLOR_TEXT, action_delete_ticket};
static list_item delete_unused_tickets = {"Borrar Tickets no usados", COLOR_TEXT, action_delete_tickets_unused};
...
static void tickets_action_open(linked_list* items, list_item* selected) {
    ...
    list_display("Accion de ticket", "A: Seleccionar, B: Volver", data, tickets_action_update, tickets_action_draw_top);
}
...
static void tickets_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Fallo al iniciar el llenado de \nla lista de tickets.");
        }
...
void tickets_open() {
    ...
    list_display("Tickets", "A: Seleccionar, B: Volver, X: Refrescar", data, tickets_update, tickets_draw_top);
}