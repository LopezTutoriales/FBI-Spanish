...
static list_item browse_user_save_data = {"Examinar partidas guardadas del usuario", COLOR_TEXT, action_browse_user_ext_save_data};
static list_item browse_spotpass_save_data = {"Examinar partidas guardadas de SpotPass", COLOR_TEXT, action_browse_boss_ext_save_data};
static list_item delete_save_data = {"Borrar partidas guardadas", COLOR_TEXT, action_delete_ext_save_data};
...
static void extsavedata_action_open(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Fallo al asignar accion para \nlos datos de guardado externos.");
    ...
    list_display("Accion de datos de guardado externos", "A: Seleccionar, B: Volver", data, extsavedata_action_update, extsavedata_action_draw_top);
}
...
static void extsavedata_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        extsavedata_options_add_entry(items, "Mostrar tarjeta SD", &listData->showSD);
        extsavedata_options_add_entry(items, "Mostrar NAND", &listData->showNAND);
        extsavedata_options_add_entry(items, "Ordenar por ID", &listData->sortById);
        extsavedata_options_add_entry(items, "Ordenar por nombre", &listData->sortByName);
    }
}

static void extsavedata_options_open(extsavedata_data* data) {
    list_display("Opciones", "A: Cambiar, B: Volver", data, extsavedata_options_update, NULL);
}
...
static void extsavedata_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(!listData->populated || (hidKeysDown() & KEY_X)) {
        ...
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Fallo al iniciar el llenado de la \nlista de datos guardados externos.");
        }
...
void extsavedata_open() {
    ...
    list_display("Datos de guardado externos", "A: Seleccionar, B: Volver, X: Resfrescar, Select: Opciones", data, extsavedata_update, extsavedata_draw_top);
}